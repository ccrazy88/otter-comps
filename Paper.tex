...
\section{Overview of occupancy modeling}
    ...
    assumption of no false positives and the independence assumption across
	plots.
...
\label{math}
    Given a binary sequence \(X_1, ..., X_n\) of independently and indentically
	distributed \(0\)'s and \(1\)'s, let \(A = \text{\# of successive 0-1's}\),
	\(B=\text{\# of successive 
	1-1's}\) and \(p = \text{ the proportion of 1's in the sequence}\).
    ...
    \begin{equation*}
        ...
        Var[A]& =\sum_{i=1}^{n-1}Var[A]+2\sum_{i=1}^{n-2}Cov[\{X_i=0, 
              ...
              & \quad +2\sum_{i=1}^{n-2}\bigg(E[\{X_i=0, X_{i+1}=1\},
			  	\{X_{i+1}=0, X_{i+2}=1\}]  \\
			  &	\qquad \qquad \quad - E[X_i=0, X_{i+1}=1]E[ X_{i+1}=0, 
				 X_{i+2}=1]\bigg) \\
    ...
    \begin{equation*}
        ...
        Var[B]& =\sum_{i=1}^{n-1}Var[B]+2\sum_{i=1}^{n-2}Cov[\{X_i=1, 
              ...
              & \quad +2\sum_{i=1}^{n-2}\bigg(E[\{X_i=1, X_{i+1}=1\},
				 \{X_{i+1}=1, X_{i+2}=1\}]  \\
			  & \qquad \qquad \quad -E[X_i=1, X_{i+1}=1]E[X_{i+1}=1,
				 X_{i+2}=1]\bigg) \\
